 ...
 */
function parseMcpResponseBody(raw) {
    ...
    if (trimmed.includes("data:") || trimmed.startsWith("event:")) {
        ...
        const fallbackJsonishLines = lines.filter((line) => line.length > 0 && !/^event:\s*/.test(line) && !/^id:\s*/.test(line) && !/^retry:\s*/.test(line));
...
async function mcpFetch(endpoint, body, options = {}) {
 ...
 */
export async function initializeMcp(endpoint, options = {}) {
    const timeoutMs = options.timeoutMs ?? 15_000;
    try {
        const initRes = await mcpFetch(endpoint, {
        ...
        const notifyHeaders = {};
        if (sessionId)
            notifyHeaders[SESSION_HEADER] = sessionId;
 ...
 */
export async function callTool(endpoint, toolName, args = {}, options = {}) {
    const id = nextId();
    const body = {
        jsonrpc: JSON_RPC_VERSION,
        id,
 ...
 */
export async function getDesignContext(endpoint, params, options = {}) {
    const args = params === "desktop"
        ? {}
        : {
 ...
 */
export async function getScreenshot(endpoint, params, options = {}) {
    const args = params === "desktop"
        ? {}
        : {
            ...(params.fileKey ? { fileKey: params.fileKey } : {}),
 ...
 */
export async function getVariableDefs(endpoint, params, options = {}) {
    const args = params === "desktop"
        ? {}
        : {
            ...(params.fileKey ? { fileKey: params.fileKey } : {}),
 ...
 */
export async function getMetadata(endpoint, params, options = {}) {
    const args = params === "desktop"
        ? {}
        : {
            ...(params.fileKey ? { fileKey: params.fileKey } : {}),
