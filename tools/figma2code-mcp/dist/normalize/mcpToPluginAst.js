 ...
 */
function metadataRowToNode(row, parent) {
    ...
    const relX = parent !== undefined
        ? row.x >= parent.x
            ? round(row.x - parent.x)
            : round(row.x)
        : undefined;
    const relY = parent !== undefined
        ? row.y >= parent.y
 ...
 */
function tryParseStructuredDesignContext(content) {
    ...
    try {
        ...
        if (data && typeof data === "object" && "tree" in data) {
            return { tree: data.tree, root: data };
        }
...
function pathKeyFromNodeId(nodeId) {
 ...
 */
function parseReactLikeDesignContext(content, fallbackRootId) {
    ...
    const parsePxFromClass = (cls, key) => {
    ...
    const parseBracketPx = (cls, prop) => {
        const extractPx = (body) => {
    ...
    const parseFontWeight = (cls) => {
        if (/\bfont-bold\b/.test(cls))
            return 700;
        if (/\bfont-semibold\b/.test(cls))
            return 600;
        if (/\bfont-medium\b/.test(cls))
            return 500;
        if (/\bfont-normal\b/.test(cls))
            return 400;
        return undefined;
    };
    const sanitizeJsxText = (s) => {
    ...
    while ((m = elementRe.exec(content)) !== null) {
        ...
        const type = tag === "p" || tag === "span" || tag === "h1" || tag === "h2" || tag === "h3" || tag === "h4"
            ? "TEXT"
            : "FRAME";
        ...
        if (node) {
            ...
            if (/\bflex\b/.test(cls)) {
                const auto = { layout: /\bflex-col\b/.test(cls) ? "VERTICAL" : "HORIZONTAL" };
                const gap = parseBracketPx(cls, "gap");
                if (typeof gap === "number")
                    auto.itemSpacing = gap;
            ...
            if (!node.h && typeof hClass === "number")
                node.h = hClass;
        }
    }
    if (!map.size)
    ...
    while ((m = textRe.exec(content)) !== null) {
        const tag = String(m[1] || "").toLowerCase();
        if (!(tag === "p" || tag === "span" || tag === "h1" || tag === "h2" || tag === "h3" || tag === "h4"))
            continue;
    ...
    for (const node of map.values()) {
        if (node.type !== "TEXT" || (node.text && String(node.text.raw || "").trim()))
            continue;
    ...
    const root = (preferredRootKey && map.get(preferredRootKey)) ||
        rootCandidates.map((k) => map.get(k)).find((n) => !!n) ||
        [...map.entries()]
    ...
    const promoteImageToChild = (n) => {
        ...
        for (const c of kids)
            promoteImageToChild(c);
    };
    if (root)
        promoteImageToChild(root);
    return root || null;
}
function collectNodesByPathKey(root) {
    const map = new Map();
    const walk = (n) => {
...
function enrichTreeFromDesignContext(tree, designContext) {
    if (!designContext)
        return tree;
    ...
    const attachHintChildrenByPrefix = (node, nodeKey) => {
    ...
    const walk = (n) => {
        ...
        if (hint) {
            ...
            if (!n.h && hint.h)
                n.h = hint.h;
            if ((!n.name || n.name === n.id) && hint.name)
                n.name = hint.name;
...
function applyCountersSectionSemanticHints(tree, slug) {
    const walk = (n, fn) => {
        fn(n);
        for (const c of n.children || [])
            walk(c, fn);
...
function applyHeroTopSectionSemanticHints(tree) {
    const findFirst = (n, pred) => {
        if (pred(n))
            return n;
        for (const c of n.children || []) {
...
function inferInteractiveStatesFromTree(tree) {
    const stateKeys = ["default", "hover", "active", "focus", "disabled"];
    const statePriority = {
        default: 0,
        hover: 1,
        active: 2,
    ...
    const walk = (parent) => {
        ...
        if (kids.length > 1) {
            ...
            for (const group of groups.values()) {
                ...
                if (!presentKeys.some((k) => k !== "default"))
                    continue;
                ...
                if (group.default && group.default !== defaultNode) {
                    ...
                    if (def) {
                        if (Array.isArray(def.fills))
                            defaultNode.fills = def.fills.map((f) => ({ ...f }));
                        if (def.stroke)
                            defaultNode.stroke = { ...def.stroke };
                        if (Array.isArray(def.shadows))
                            defaultNode.shadows = def.shadows.map((s) => ({ ...s }));
                        if (typeof def.opacity === "number")
                            defaultNode.opacity = def.opacity;
                        if (def.blendMode)
                            defaultNode.blendMode = def.blendMode;
                        if (def.blur)
                            defaultNode.blur = { ...def.blur };
                        if (def.r)
                            defaultNode.r = { ...def.r };
                    }
                }
 ...
 */
export function normalizeMcpToPluginAst(mcp, opts) {
    const slug = opts.slug;
    const type = (opts.type ?? "flexi_block");
    ...
    if (fromStructured?.tree && fromStructured.tree.id && Array.isArray(fromStructured.tree.children)) {
        ...
        const pickFallbackParentId = (child, candidates) => {
            let best;
            for (const p of candidates) {
                if (p.id === child.id)
                    continue;
                if (p.width < child.width || p.height < child.height)
                    continue;
                const containsAbs = child.x >= p.x &&
                    child.y >= p.y &&
                    child.x + child.width <= p.x + p.width &&
                    child.y + child.height <= p.y + p.height;
                const containsRel = child.x >= 0 &&
                    child.y >= 0 &&
                    child.x + child.width <= p.width &&
                    child.y + child.height <= p.height;
                if (!containsAbs && !containsRel)
                    continue;
        ...
        if (rows.length === 0) {
            ...
            for (let i = 0; i < rows.length; i++) {
                const row = rows[i];
                if (row.id === rootRow.id)
                    continue;
                const explicitParentId = row.parentId && nodeById.has(row.parentId) ? row.parentId : undefined;
                const inferredParentId = explicitParentId ||
                    pickFallbackParentId(row, rows
                        .slice(0, i)
...
//# sourceMappingURL=mcpToPluginAst.js.map