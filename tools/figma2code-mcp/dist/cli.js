...
import { requireNodeId } from "./figma/url.js";
...
async function runIngest(flags) {
    ...
    if (designRes.content && (designRes.content.includes("truncated") || designRes.content.length > 100_000)) {
        ...
        if (metaRes.content)
            metadataContent = metaRes.content;
    }
    else {
        const metaRes = await getMetadata(mcpEndpoint, mcpParams, { timeoutMs: 15_000 });
        if (metaRes.content)
    ...
    const metadataUsable = metadataContent &&
    ...
    const type = (flags.type === "navbar" || flags.type === "footer" ? flags.type : "flexi_block");
    ...
    if (!flags.slug) {
        const inferredName = String(ast?.tree?.name || "").trim() ||
            String(ast?.meta?.figma && typeof ast.meta.figma === "object" ? ast.meta.figma.frameName || "" : "").trim();
        if (inferredName) {
            ...
            if (ast.meta && typeof ast.meta.figma === "object") {
                ast.meta = {
                    ...ast.meta,
                    figma: { ...ast.meta.figma, frameName: inferredName },
                };
            }
        }
    }
    const isEmptyTree = !ast.tree?.children?.length && (ast.tree?.w === 0 || !ast.tree?.w) && (ast.tree?.h === 0 || !ast.tree?.h);
    if (isEmptyTree) {
    ...
    if (flags.post && !flags.dryRun) {
        ...
        if (result.ok) {
            ...
            if (result.json && typeof result.json === "object" && "previewUrl" in result.json) {
                console.error("Preview: " + result.json.previewUrl);
            }
        }
        else {
